...
без замера RAM и влияния на retrieval API. Временные файлы OCR направлены в
`/app/storage/parsed/.ocr-tmp`, потому что защищённый tmpfs `/tmp` ограничен 512 MB.
Перед OCR сервис требует не менее `OCR_MIN_TEMP_FREE_MB=4096` свободного места.
